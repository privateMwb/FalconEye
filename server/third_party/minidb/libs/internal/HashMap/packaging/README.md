...
`packaging/recipes/hashmappro/all/conanfile.py`). Scoped to what isn't obvious
...
- **Don't assume the release tarball's extracted folder name matches
  the CMake project/library target name** — `cd` into whatever the
  tarball actually produced, not the library name. If you don't have a
  published release yet, use the local repo checkout directly instead
  (see step 1 below) — no need to cut a release just to verify
...
```bash
find $HOME/staging/lib/cmake/HashMapPro -name "*.cmake"
```

Expect `HashMapProConfig.cmake`, `HashMapProConfigVersion.cmake`,
`HashMapProTargets.cmake` (plus a per-config
`HashMapProTargets-<config>.cmake`) under `lib/cmake/HashMapPro/`.
...
target_link_libraries(test PRIVATE HashMapPro::HashMapPro)
EOF

cat > main.cpp << 'EOF'
#include <HashMapPro/HashMap.h>
int main() {
    rain::HashMap<int, int> map{16};
    return 0;
}
EOF
...
just constructs a `rain::HashMap<int, int>` and returns, so success
...
```bash
conan create packaging/recipes/hashmappro/all --version 1.0.0 \
...
header-only library — a single `package_id` shared across every
settings combination rather than one computed per combination.

This step also automatically builds and runs
`packaging/recipes/hashmappro/all/test_package/` against the freshly-built
package — a `HashMapPro linked and constructed successfully.` line in
